...
class Common {
    ...
    static function __init__() {
        ...
        decodeMap = {"0":65533,"128":8364,"130":8218,"131":402,"132":8222,"133":8230,"134":8224,"135":8225,"136":710,"137":8240,"138":352,"139":8249,"140":338,"142":381,"145":8216,"146":8217,"147":8220,"148":8221,"149":8226,"150":8211,"151":8212,"152":732,"153":8482,"154":353,"155":8250,"156":339,"158":382,"159":376};
        var legacy = Reflect.fields(legacyMap);
        var keys = Reflect.fields(entityMap);
        ...
        decodeHTML_re = new EReg("&(?:" + keys.join("|") + "|#[xX][\\da-fA-F]+;?|#\\d+;?)", "g");
    }

    static function makeEncodeCache(exclude:String):Array<String> {
        var encodeCache = [];

        for (i in 0...128) {
            var ch = String.fromCharCode(i);

            if (~/^[0-9a-z]$/i.match(ch)) {
                // always allow unencoded alphanumeric characters
                encodeCache.push(ch);
            } else {
                var s = '0' + StringTools.hex(i).toUpperCase();
                encodeCache.push('%' + s.substr(s.length - 2));
            }
        }

        for (i in 0...exclude.length)
            encodeCache[exclude.charCodeAt(i)] = exclude.charAt(i);

        return encodeCache;
    }

    static function makeDecodeCache(exclude:String):Array<String> {
        var decodeCache = [];
        ...
        for (i in 0...exclude.length) {
            ...
            decodeCache[ch] = '%' + s.substr(s.length - 2);
        }

        return decodeCache;
    }

    static function decodeCodePoint(codePoint:Int):String {
        if ((codePoint >= 0xD800 && codePoint <= 0xDFFF) || codePoint > 0x10FFFF)
            return "\uFFFD";

        var key = Std.string(codePoint);
        if (Reflect.hasField(decodeMap, key))
            codePoint = Reflect.field(decodeMap, key);
    ...
    public static inline function decodeHTML(s:String):String {
        return decodeHTML_re.map(s, decodeHTML_map);
    }
    ...
    // encoded sequences.
    static function encodeURI(string:String):String {
        var result = '', ii = 0, l = string.length;
        while (ii < l) {
            var i = ii;
            ii++;
    ...
    static function decodeURI(string:String):String {
        return ~/(%[a-f0-9]{2})+/gi.map(string, function(r) {
            var seq = r.matched(0);
            var ii = 0, l = seq.length, result = '';
            ...
            while (ii < l) {
                ...
                if ((b1 & 0xF0) == 0xE0 && (i + 6 < l)) {
                    ...
                    if ((b2 & 0xC0) == 0x80 && (b3 & 0xC0) == 0x80) {
                        var char = ((b1 << 12) & 0xF000) | ((b2 << 6) & 0xFC0) | (b3 & 0x3F);
                ...
                if ((b1 & 0xF8) == 0xF0 && (i + 9 < l)) {
                    ...
                    if ((b2 & 0xC0) == 0x80 && (b3 & 0xC0) == 0x80 && (b4 & 0xC0) == 0x80) {
                        var char = ((b1 << 18) & 0x1C0000) | ((b2 << 12) & 0x3F000) | ((b3 << 6) & 0xFC0) | (b4 & 0x3F);

                        if (char < 0x10000 || char > 0x10FFFF) {
                            result += '\ufffd\ufffd\ufffd\ufffd';
                        } else {
                            char -= 0x10000;
                            result += String.fromCharCode(0xD800 + (char >> 10));
                            result += String.fromCharCode(0xDC00 + (char & 0x3FF));
                        }
    ...
    public static function normalizeURI(uri:String):String {
        return try encodeURI(decodeURI(uri)) catch (_:Dynamic) uri;
    }
